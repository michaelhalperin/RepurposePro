...
function CopyButton({ text, small, label }) {
  ...
  if (small) {
    return (
      <button className={`copy-btn-sm ${copied ? 'copied' : ''}`} onClick={handleCopy} title="Copy">
        {copied ? '✓' : 'Copy'}
      </button>
    )
  }
...
function SaveButton({ saved, onSave, onUnsave, loading }) {
  return (
    ...
    >
      <Bookmark size={15} strokeWidth={1.75} fill={saved ? 'currentColor' : 'none'} />
    </button>
  )
}
  ...
  return (
    <article className="card tiktok-card">
      <div className="card-header">
        <div className="card-badge card-badge--tiktok">TikTok · Idea {index + 1}</div>
        <div className="card-actions">
          <SaveButton saved={saved} onSave={onSave} onUnsave={onUnsave} loading={saving} />
          ...
          <li key={i}>
            {editingIndex === i ? (
              <div className="tweet tweet--editing">
                ...
                <div className="tweet-edit-wrap">
                  ...
                  <div className="tweet-footer">
                    <span className={`tweet-char-count ${tweet.length > 280 ? 'tweet-char-count--over' : ''}`}>
                      {tweet.length}/280
                    </span>
                    <button className="tweet-done-btn" onClick={() => setEditingIndex(null)}>Done</button>
                  </div>
                </div>
              </div>
            ) : (
              <div
                ...
                aria-label={copiedIndex === i ? `Post ${i + 1} copied to clipboard` : `Copy post ${i + 1} to clipboard`}
              >
                <span className="tweet-num" aria-hidden="true">{i + 1}</span>
...
export default function ResultsScreen({ output, onOutputChange, onDelete, deleting, savedItems, onSaveItem, onUnsaveItem }) {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from('.results-hero', { opacity: 0, y: -12, duration: 0.4, ease: 'power2.out' })
  ...
  const savedMap = useMemo(() => {
    ...
    savedItems.forEach(item => {
      if (item.item_type === 'tiktok') {
        output.tiktoks?.forEach((t, i) => {
          if (JSON.stringify(t) === JSON.stringify(item.item_data)) {
            map[`tiktok-${i}`] = item.id
          }
        })
      } else if (item.item_type === 'thread') {
        if (JSON.stringify({ tweets: output.thread }) === JSON.stringify(item.item_data)) {
          map['thread'] = item.id
        }
      } else if (item.item_type === 'linkedin') {
        if (JSON.stringify({ post: output.linkedin }) === JSON.stringify(item.item_data)) {
  ...
  function handleEditTikTok(index, field, value) {
    const newTiktoks = output.tiktoks.map((t, i) =>
      i === index ? { ...t, [field]: value } : t
    )
    onOutputChange({ ...output, tiktoks: newTiktoks })
  }

  function handleEditTweet(index, value) {
    const newThread = output.thread.map((t, i) => i === index ? value : t)
    onOutputChange({ ...output, thread: newThread })
  }

  function handleEditLinkedIn(value) {
    onOutputChange({ ...output, linkedin: value })
  }

  const nTiktok = output.tiktoks?.length ?? 0
  const nThread = output.thread?.length ?? 0

  const allTikTokText = output.tiktoks.map((t, i) =>
  ...
  return (
    <div className="results-screen" ref={containerRef}>
      <div className="results-inner">
        <header className="results-hero">
          <div className="results-hero-text">
            ...
            <p className="results-sub">
              {nTiktok} TikTok {nTiktok === 1 ? 'script' : 'scripts'} · {nThread} thread {nThread === 1 ? 'post' : 'posts'} · 1 LinkedIn draft
            </p>
          </div>
          <div className="results-hero-actions">
            {confirmDelete ? (
              <div className="confirm-inline">
                <span className="confirm-text confirm-danger">Delete this result?</span>
                <button className="confirm-yes danger" onClick={onDelete} disabled={deleting}>
        ...
        <section className="results-section results-section--tiktok" aria-labelledby="results-tiktok-heading">
          <div className="results-section-head">
            <div>
              <h2 id="results-tiktok-heading" className="section-heading">TikTok</h2>
              <p className="section-hint">Short-form hooks and scripts · click any field to edit</p>
            </div>
            <CopyButton text={allTikTokText} label="Copy all" />
          </div>
          <div className="cards-grid">
            {output.tiktoks.map((t, i) => {
              const key = `tiktok-${i}`
              return (
                <TikTokCard
                  key={i}
        ...
        <div className="results-two-col">
          <section className="results-section results-section--thread" aria-labelledby="results-thread-heading">
            <div className="results-section-head">
              <div>
                <h2 id="results-thread-heading" className="section-heading">X thread</h2>
                <p className="section-hint">Ordered for posting</p>
              </div>
            </div>
            <ThreadBlock
              tweets={output.thread}
              saved={!!savedMap['thread']}
              onSave={() => handleSave('thread', 'thread', { tweets: output.thread })}
          ...
          <section className="results-section results-section--linkedin" aria-labelledby="results-li-heading">
            <div className="results-section-head">
              <div>
                <h2 id="results-li-heading" className="section-heading">LinkedIn</h2>
                <p className="section-hint">Long-form post · click to edit</p>
              </div>
            </div>
            <LinkedInBlock
              post={output.linkedin}
              saved={!!savedMap['linkedin']}
              onSave={() => handleSave('linkedin', 'linkedin', { post: output.linkedin })}
